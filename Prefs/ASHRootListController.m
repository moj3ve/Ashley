#include "ASHRootListController.h"

@implementation ASHRootListController

- (instancetype)init {
    self = [super init];

    if (self) {
        ASHAppearanceSettings *appearanceSettings = [[ASHAppearanceSettings alloc] init];
        ...
        self.iconView.image = [UIImage imageWithContentsOfFile:@"/Library/PreferenceBundles/AshleyPrefs.bundle/icon@2x.png"];
...
- (void)viewDidLoad {
    ...
    self.headerImageView.image = [UIImage imageWithContentsOfFile:@"/Library/PreferenceBundles/AshleyPrefs.bundle/Banner.png"];
...
-(void)respring {
	UIAlertController *respring = [UIAlertController alertControllerWithTitle:@"Ashley"
...
- (void)showThanksAlert {
    ...
    NSString* pathForAshleyPlist = @"/var/mobile/Library/Preferences/me.shymemoriees.ashleypreferences";

    if ([fileManager fileExistsAtPath:pathForAshleyPlist]) {
        UIAlertController *alert = [UIAlertController alertControllerWithTitle:@"Ashley"
        message:@"Thanks For Downloading Ashley!\nI hope you enjoy it 😊\n\n[Toggle Anything In The Prefs To Make This Alert Not Show Anymore]"
        preferredStyle:UIAlertControllerStyleAlert];
