...
if (enabled && allowAshleyToTalkSwitch && phraseNumber == 0) {
  SystemSoundID helloPhrase = 0;
  AudioServicesDisposeSystemSoundID(helloPhrase);
  AudioServicesCreateSystemSoundID((CFURLRef) CFBridgingRetain([NSURL fileURLWithPath:@"/Library/Ashley/helloPhrase.caf"]), &helloPhrase);
  AudioServicesPlaySystemSound((SystemSoundID)helloPhrase);

} else if (enabled && allowAshleyToTalkSwitch && phraseNumber == 1) {
  SystemSoundID cancelPhrase = 0;
  AudioServicesDisposeSystemSoundID(cancelPhrase);
  AudioServicesCreateSystemSoundID((CFURLRef) CFBridgingRetain([NSURL fileURLWithPath:@"/Library/Ashley/cancelPhrase.caf"]), &cancelPhrase);
  AudioServicesPlaySystemSound((SystemSoundID)cancelPhrase);

} else if (enabled && allowAshleyToTalkSwitch && phraseNumber == 2) {
  SystemSoundID confirmPhrase = 0;
  AudioServicesDisposeSystemSoundID(confirmPhrase);
  AudioServicesCreateSystemSoundID((CFURLRef) CFBridgingRetain([NSURL fileURLWithPath:@"/Library/Ashley/confirmPhrase.caf"]), &confirmPhrase);
...
@class CSTeachableMomentsContainerView; @class SBUICallToActionLabel; @class SBUIPasscodeLockViewWithKeypad; @class SBIconController; @class SBCoverSheetPrimarySlidingViewController; @class NCNotificationListSectionRevealHintView; 
